...
this.initialize = function(app) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        // save the device connection in connection pool
        ...
        socket.on('sd_exception', function (actionId, methodCallId, device_id, exception_str) {
            log('sd_exception');

            log('sd_exception for action: ' + actionId);
            ...
            log('sd_exception end');
        });



        socket.on('sd_event', function (actionId, device_id, event_value) {
            log('sd_event');

            log('sd_event for action: ' + actionId);
    ...
    DEVICE_HANDLER.findAllDevices(function(err, devices) {
        ...
        for(i in devices) {
            ...
            for (var i = 0; i < capabilityNames.length; i++) {
                ...
                capabiltitiesAndColors.push({"color": HELPERS.hashCode(capa),"name":capa});
            }
            tempDev['capabilities'] = capabiltitiesAndColors;
            
            tempDev['status'] = 'offline';
            tempDev['statusColor'] = 'FF5050';
            if(CONNECTION_POOL[devModel.identity]) {
                tempDev['status'] = 'online';
                tempDev['statusColor'] = '00FF99';
            }
            response.push(tempDev);
        }
        res.writeHead(200, {"Content-Type": "application/json"});
        res.write(
            JSON.stringify({"devices":response})
        );
        res.end();
    });
};
...
this.getActionInstances = function(req, res) {
    ...
    res.writeHead(200, {"Content-Type": "application/json"});
    res.write(
        JSON.stringify({"actioninstances":response})
    );
    res.end();
};
...
// TODO: set here timeout which throws exception or creates error if devices not responding for a long time
        ...
        while(runRetType != runYieldHelpers.METHOD_CALL_RESPONSE && runRetType != runYieldHelpers.SD_EXCEPTION) {
            try {
                ...
                // handle exceptions
                } else if(runRet['runRetType'] && runRet['runRetType'] == runYieldHelpers.SD_EXCEPTION) {
                    runRetType = runRet['runRetType'];
                    break;
                }
            }catch(e) {log(e);}
...
*/
    ...
    */
    ...
    this.serverSideExceptionHandler =  function(exception_value) {log('default server-side exception handling because of: '+exception_value);};
    this.handleServerSideException = function(exception_value) {
        ...
        var runResponse = {runRetType: runYieldHelpers.SD_EXCEPTION};
    ...
    */
    ...
    this.exceptionHandler =  function(action, device, exception_value) {log('handling exception');};
    this.handleException = function(methodCallId, device_id, exception_value) {
        ...
        this.exceptionHandlerFiber = Fiber(function() {
            try {
                ...
                var runResponse = {runRetType: runYieldHelpers.SD_EXCEPTION, methodCallId: methodCallId, runRetValue: undefined};
    ...
    this.eventHandler =  function(action, device, event_value) {log('handling event');};
    this.handleEvent = function(device_id, event_value) {
        ...
        var runResponse = {runRetType: runYieldHelpers.SD_EVENT};
        this.eventHandlerFiber.run(runResponse);
    };
...
function removeActionInstance(actionId) {
    try{
        ...
        if(actionPool[actionId]) {
            ...
            }catch(ww){log('cannot destroy: '+ww);}
