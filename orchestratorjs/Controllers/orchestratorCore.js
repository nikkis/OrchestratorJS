...
var ActionInstanceDataHandler = require(ROOT + '/Models/actionInstanceData');
var ACTION_INSTANCE_DATA_HANDLER = new ActionInstanceDataHandler();
...
this.initialize = function (app) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('ojs_context_data', function (actionId, deviceId, metadataDict) {
            log('ojs_context_data: ' + metadataDict);
    ...
    try {
        ...
        for (i in deviceModels) {
            participantInfo.push({
                identity: deviceModels[i].identity,
                capabilities: deviceModels[i].capabilities,
                btUUID: deviceModels[i].btUUID,
                username: deviceModels[i].username
            });
        ...
        var params = {
            ...
            actionVersionHash: actionVersionHash
        };
    ...
    try {
        ...
        function replaceIdsWithDevices(paramsArray) {
            for (i in paramsArray) {
                ...
                } else if (typeof param == 'string' && param.slice(0, 7) == 'device:') {
